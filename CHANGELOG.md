...
- **Rate Limits page** - rate limiting used to be invisible: a sender ran out of allowance, you got a "rate limit hit" mail, and there was nothing in the viewer that told you who it was or why. The new page groups the collected ratelimited log by sender, with the number of hits, the latest recipients and subjects, and the limit each sender is actually configured with, so the noisy newsletter or the compromised mailbox is obvious at a glance. A sender stuck behind a counter can be released with one click, which lets them send again immediately without changing their limit. Mailbox and domain limits can also be viewed and edited straight from the viewer instead of switching to the mailcow UI
