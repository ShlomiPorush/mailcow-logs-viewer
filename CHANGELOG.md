...
- **MTA-STS check on the Domains page** ([#83](https://github.com/ShlomiPorush/mailcow-logs-viewer/issues/83)) - each domain now also shows whether it publishes an MTA-STS policy (RFC 8461), which tells sending servers to require TLS when delivering to you. The check validates the `_mta-sts` DNS record, fetches the policy file behind it, and verifies your MX hosts are covered - an enforced policy with an uncovered MX is flagged as an error because senders will refuse to deliver through it. MTA-STS changes also fire the existing DNS change alert. Domains without MTA-STS get an informational note, never an error. Thanks to [@q16marvin](https://github.com/q16marvin) for the request
- **Messages dropped by a Sieve rule now show "Discarded" instead of a false "Delivered"** ([#65](https://github.com/ShlomiPorush/mailcow-logs-viewer/issues/65)) - message tracking used to end at Postfix, which logs status=sent the moment it hands a message to Dovecot, so a message a Sieve `discard` rule dropped was reported as delivered. The Dovecot LMTP lines the Live Logs collector already ingests are now correlated into each message (no extra mailcow API calls): a discarded message gets its own `discarded` status with badge and filter, the message dialog shows the mailbox-delivery outcome (target folder, forward target, reject reason, storage errors like a full quota) along with the raw Dovecot lines, and the message list hints when Sieve filed a message into a folder other than the inbox. Requires raw log collection with the `dovecot` service enabled. Thanks to [@mrclschstr](https://github.com/mrclschstr) for the report and to [@Meeppoo](https://github.com/Meeppoo) for the implementation groundwork this builds on
- **mailcow alias domains are recognized across the viewer** ([#92](https://github.com/ShlomiPorush/mailcow-logs-viewer/issues/92)) - a domain configured in mailcow as an alias of another was invisible before: mail sent as `user@alias.tld` appeared nowhere in Mailbox Statistics, and the alias domain's own DNS records were never checked. The alias mapping is now synced from mailcow. Mailbox Statistics attributes `user@alias.tld` traffic to the mailbox `user@target.tld` as a "domain alias" row in its aliases table, counts it into the combined totals exactly once, lists alias domains in the domain filter, and the summary cards include the traffic. The Domains page shows each alias domain inside its target with its own SPF, DKIM and DMARC validation, included in scheduled checks and change alerts. Thanks to [@Neocridas](https://github.com/Neocridas) for the report
- **Rate Limits view on the Mailbox Stats page** - rate limiting used to be invisible: a sender ran out of allowance, you got a "rate limit hit" mail, and there was nothing in the viewer that told you who it was or why. Mailbox Statistics now has a Rate Limits view alongside Statistics, split into three tabs. **Recent hits** groups the collected ratelimited log by sender, with the number of hits, the latest recipients and subjects, and the limit each sender is actually configured with, so the noisy newsletter or the compromised mailbox is obvious at a glance; a sender stuck behind a counter can be released with one click, which lets them send again immediately without changing their limit. **Mailboxes** and **Domains** list the configured limits and let you edit them straight from the viewer instead of switching to the mailcow UI. The same limit can also be applied to a whole selection in one go: narrow the configured limits with the search box and the type filter, press **Apply to filtered**, and the limit you enter is set on everything the table currently shows after a single confirmation - a value of 0 takes the limit off all of them instead, and names mailcow no longer knows are skipped rather than failing the batch. Like every other feature it can be switched off from Settings, with the feature id `rate-limits`
- **Rate Limits stands on its own** - it lives on the Mailbox Stats page but no longer depends on that feature being switched on. With `mailbox-stats` disabled and `rate-limits` enabled the tab stays in the menu, renamed to Rate Limits, opens straight on the rate limits view, and the background sync that collects each mailbox's configured limit keeps running - so the limits stay current and new mailboxes can still be edited. The alias statistics job remains tied to Mailbox Stats alone

### Changed

- **Switching a feature off now clears the data it leaves behind** - a disabled feature used to keep its rows in the database forever, invisible and never refreshed. Turning off Mailbox Stats now removes the collected alias statistics, and turning off Rate Limits removes its record of who reset which counter. The mailbox table is shared by the two features, so it is only removed once both are off. The cleanup runs on startup and immediately when you change the setting from the Settings page

### Fixed

- **A suppressed address could block innocent recipients** - the Rspamd suppression map is a regexp map, but entries were written as bare addresses, so rspamd compiled them unanchored: suppressing e@example.com also rejected mail to alice@example.com, joe@example.com and every other recipient that merely contains the suppressed string. Entries are now written as anchored, escaped patterns, domain suppressions get an end anchor too (previously user@example.com.evil.net matched a suppression for example.com), and the whole map - including manually added bare addresses - migrates to the safe format automatically on the next sync, with the maps editor anchoring bare addresses on save as well. No operator action is needed
- **Dashboard message timeline was silently empty** - the hourly timeline chart's query built its spam counter with an invalid SQLAlchemy expression, so the endpoint always failed and returned an empty timeline with a logged error instead of data. Spotted in [@Meeppoo](https://github.com/Meeppoo)'s branch for #65
- **Mail between two hosted domains was called "internal" even when it arrived from outside** - the direction check looked only at the sender and recipient domains, so a message for a hosted domain that was sent through an outside relay (for example Microsoft 365) and entered from the internet was labeled internal instead of inbound. Internal now also requires a local origin: an authenticated submission or a private source address
...
- **A forwarded message hid the delivery it came from** ([#36](https://github.com/ShlomiPorush/mailcow-logs-viewer/issues/36)) - when a SOGo filter, a Sieve redirect or any other re-submission sent a message on, both deliveries carried the same Message-ID, and messages were tracked by Message-ID alone. The second delivery overwrote the first one, so the Messages list showed only the forward, with its sender, recipient and status - the original incoming message was gone, and a forward that Rspamd rejected made a message that had actually arrived look rejected. Each delivery is now tracked on its own, so both appear in Messages with their own sender, recipient, direction and status, and the message dialog lists the related deliveries of the same message with a link between them. Existing links to a message keep working. Thanks to [@piperino721](https://github.com/piperino721) for the report
- **Mailbox Statistics counted a forwarded message several times** ([#36](https://github.com/ShlomiPorush/mailcow-logs-viewer/issues/36)) - now that every delivery of a message is tracked on its own, a mailbox that forwards its mail was credited with sending each forward as if it were mail of its own, and a forward attempt that failed raised its failure rate even when the message was delivered a moment later. Each mailbox now counts a message once, however many deliveries it took, and the most successful delivery is the one that counts, so a failed attempt is no longer held against a mailbox whose message was delivered in the end. The summary cards at the top of the page count the same way
